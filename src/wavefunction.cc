					   ...
					   int propornot[])
{
  ...
  for (zindex=0; zindex<g.ngps_z(); zindex++) 
      ...
      if ((propornot[zindex]==1) || (time<=0.0))
	{
	  m=ms[zindex];
	  mm=m*m; 
	  //	  cout << "Propagating orbital no. " << zindex << " with m = " << m << endl;
	      ...
	      for (z=-g.r(g.ngps_x())*fact; z<g.r(g.ngps_x())*fact; z+=g.delt_x()*stepwidth)
		{
		  ...
		  if (xindex<g.ngps_x())
		      ...
		      for (yindex=(long)(0); yindex<g.ngps_y(); yindex++)
			{
			  ...
			  summingres=summingres+start[i]*ylm(yindex,0,acos(u),0.0);
			};
...
//   cout << j << ", " << i << " -> clebschsum: " << clebschsum << endl;
...
wavefunction wavefunction::calculate_U_ji_l(grid g, long j, long i, long order)
{
  ...
  switch (order)
	...
	{      
      ...
      break;
    default :
      cout << "I AM IN THE U_IJ LOOP WITH POW" << endl;
      rindex=0;
      r=g.r(rindex);
      for (rrindex=0; rrindex<g.ngps_x(); rrindex++)
	{
...
complex wavefunction::project(grid g, grid gorig, wavefunction &orig, long zindex)
{
  ...
  if ((g.ngps_z()>1) || (gorig.ngps_z()>1))
    {
      cerr << "err: attempt to call complex wavefunction::project() with ngpsz>1" << endl;
      exit(-100);
    }
  if (g.delt_x() != gorig.delt_x())
    {
      cerr << "err: unequal delta r in complex wavefunction::project()." << endl;
      exit(-100);
    }
  if (g.dimens() != gorig.dimens())
    {
      cerr << "err: unequal propagation modes for the two grids in complex wavefunction::project()." << endl;
      exit(-100);
    }
...
complex wavefunction::expect_z(grid g)  // attention!!! works for m=0 states only because m-dependence is suppressed
{
  complex result(0.0,0.0);
  long xindex, yindex, zindex;
...
complex wavefunction::accel_z(grid g, int m, double charge)
{
  complex result(0.0,0.0);
  long xindex, yindex, zindex;
  long index, index_ym, index_yp;
	...
	{ 
	  for (xindex=0; xindex<g.ngps_x(); xindex++)
	    { 
	      ...
	      result=result+conj(start[index_yp])*(complex)(-(1.0+a*exp(-b*r))/(r*r)-a*b*exp(-b*r)/r)*start[index]*(complex)sqrt((yindex+1.0)*(yindex+1.0)-m*m)/sqrt((2.0*yindex+1.0)*(2.0*yindex+3.0))*(complex)g.delt_x();
	      ...
	      for (yindex=1; yindex<g.ngps_y()-1; yindex++)
		{
		  ...
		  result=result+(complex)g.delt_x()*(complex)(-(1.0+a*exp(-b*r))/(r*r)-a*b*exp(-b*r)/r)*start[index]
		    *(conj(start[index_yp])*(complex)sqrt((yindex+1.0)*(yindex+1.0)-m*m)/sqrt((2.0*yindex+1.0)*(2.0*yindex+3.0))
		      +conj(start[index_ym])*(complex)sqrt(yindex*yindex-m*m)/sqrt((2.0*yindex-1.0)*(2.0*yindex+1.0)));
	      ...
	      result=result+(complex)g.delt_x()*(complex)(-(1.0+a*exp(-b*r))/(r*r)-a*b*exp(-b*r)/r)*start[index]*conj(start[index_ym])*(complex)sqrt(yindex*yindex-m*m)/sqrt((2.0*yindex-1.0)*(2.0*yindex+1.0));
